...
# and also defines exception classes raised for invalid input and
# over-constrained puzzles. This module also defines the method Sudoku.solve
# to solve a puzzle. The solve method uses the Sudoku.scan method, which is
# also defined here.
#
# Use this module to solve Sudoku puzzle with code like this:
# 
# require 'sudoku' 
# or
...
module Sudoku
  ...
  # - The nine 3x3 subgrids are known as 'boxes' and are also numbered from 0
  # to 8, ordered from left to right and top to bottom. The box in the
  # upper-left is box 0. The box in the upper-right is box 2. The box in the
  # middle is box 4. The box in the lower-right is box 8.
  #
  # Create a new puzzle with Sudoku::Puzzle.new, specifying the initial state
  # as a string or as an array of strings. The string(s) should use the
  # characters 1 through 9 for the given values, and '.' for cells whose values
  # is unspecified. Whitespace in the input is ignored.
  #
  ...
  # These methods use numbers (not characters) 0 to 9 for cell contents. 0
  # represents an unknown value.
  #
  # The has_duplicates? predicate returns true if the puzzle is invalid because
  # any row, column, or box includes the same digit twice.
  #
  # The each_unknown method is an iterator that loops through the cells of the
  # puzzle and invokes the associated block once for each cell whose value is
  # unknown.
  #
  # The possible method returns an array of integers in the range 1..9.
  # The elements of the array are the only values allowed in the specified cell.
  # If this array is empty, then the puzzle is over-specified and cannot be
  # solved. If the array has only one element, then that element must be the
  # value for that cell of the puzzle.
  #
  ...
  class Puzzle
    ...
    # puzzle as an array of lines or as a single string. Use ASCII digits 1 to
    # 9 and use the '.' character for unknown cells. Whitespaces, including
    # newlines, will be stripped.
    def initialize(lines)
      if (lines.respond_to? :join) # If argument looks like an array on lines
      ...
      # Remove whitespace (including newlines) from the data. The '!' in gsub!
      # indicates that this is a mutator method that alters the string
      # directly rather than making a copy.
      s.gsub!(/\s/, "") # /\s/ is a Regexp that matches any whitespace
      
      # Raise an exception if the input is the wrong size. Note that we use
      # unless instead of if, and use it in modifier form.
      raise Invalid, "Grid is the wrong size" unless s.size === 81 # 9 x 9   
      ...
      # The following two lines convert our string of ASCII characters to an
      # array of integers, using two powerful String methods.
      ...
      raise Invalid, "Initialize puzzle has duplicates" if has_duplicates?      
    end
    ...
    def to_s
      ...
      # like this is probably not good coding style, but it demonstrates the
      # power and expressiveness of the language.
      #
      # Broken down, the line below works like this:
      # (0..8).collect invokes the code in curly braces 9 times--once for each
      # row--and collects the return value of that code into an array.
      # The code in curly braces takes a subarray of the grid representing a
      # single row and packs its numbers into a string.
      # The join() method joins the elements of the array into a single string
      # with newlines between them. Finally the tr() method translates the
      # binary string representation into ASCII digits.
    ...
    # We override the array access operator to allow access to the individual
    # cells of a puzzle. Puzzles are two-dimensional, and must be indexed with
    ...
    # This method allows the array access operator to be used on the lefthand
    # side of an assignment operation. It sets the value of the cell at
    # (row,col) to newvalue.
    ...
    # capital letter, so this is a constant. Also, the array has been frozen,
    ...
    # This method defines a custom looping construct (an 'iterator') for Sudoku
    # puzzles. For each cell whose value is unknown, this method passes
    # ('yields') the row number, column number, and box number to the block
  ...
  # setting a cell alters the possible values for other cells, it continues
  # scanning until it has scanned the entire puzzle without finding any cells
  ...
  # column of a cell whose value is still unknown. The third value is the set
  # of values possible at that row and column. This is a minimal set of
  # possible values: there is no unknown cell in the puzzle that has fewer 
  # possible values. This complex return value enables a useful heuristic in
  # the solve() method: that method can guess at values for cells where the
  # guess is most likely to correct.
  #
  # This method raises Impossible if it finds a cell for which there are no
  # possible values. This can happen if the puzzle is over-constrained, or if
  # the solve() method below has made an incorrect guess.
  #
  # This method mutates the specified Puzzle object in place. If has_duplicates?
  # is false on entry, then it will be false on exit.
  #
  ...
  # Solve a Sudoku puzzle using simple logic, if possible, but fall back on
  # brute-force when necessary. This is a recursive method. It either returns
  # a solution or raises an exception. The solution is returned as a new
  # Puzzle object with no unknown cells. This method does not modify the
  # Puzzle it is passed. Note that this method cannot detect an
  # under-constrained puzzle.
  def Sudoku.solve(puzzle)
    ...
    # It returns a row, a column, and set of possible values of that cell.
    ...
    # Since we're picking from a set of possible values, the guess leaves the
    # puzzle in a valid state. The guess will either lead to a solution or to an
    # impossible puzzle. We'll know we have an impossible puzzle if a recursive
    # call to scan throws an exception. If this happens we need to try another
    # guess, or re-raise an exception if we've tried all the options we've got.
    p.each do |guess| # For each value in the set of possible values
      ...
      begin
        ...
        # Remember that solve() will either return a valid solution or raise
        # an exception.
    ...
    # If we get here, then none of our guesses worked out so we must have
    # guessed wrong sometime earlier.
    raise Impossible      
  end
