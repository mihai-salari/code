...
base_dir="/tmp/mail_replication"
bin_dir="$base_dir/bin"
etc_dir="$base_dir/etc"
log_dir="$base_dir/log"
log_archive_dir="$log_dir/archive"

required_directories="$base_dir $bin_dir $etc_dir $log_dir $log_archive_dir"
...
for d in $required_directories; do
	echo "DEBUG: $d is required"
	
	if [ -d "$d" ]; then
		echo "DEBUG: $d found"
	else
		echo "DEBUG: $d not found, creating $d"
		mkdir $d
		chmod 755 $d
	fi
done
